...
fetch('./data/photographers.json')
    ...
    .then(data => {
        ...
        function changeImage() {
            ...
            let dataMedia = [...medias]
            let selectedIndex = 0;
            console.log(dataMedia);
            console.log(allMedia.video);



            allMedia.forEach(media => {
                media.addEventListener('click', (e) => {
                    const title = media.nextElementSibling.querySelector('.title').textContent;
                    console.log(title);
                    selectedIndex = dataMedia.findIndex(data => data.title === title);
                    console.log(selectedIndex);
                });
            });


            /*let sortedDataMedia2 = [];

            for (let i = 0; i < dataMedia.length; i++) {
                let added = false;
                for (let j = 0; j < allMedia.length; j++) {
                    if (dataMedia[i].title === allMedia[j].alt) {
                        sortedDataMedia2.push(allMedia[j]);
                        added = true;
                        break;
                    }
                }
                if (!added) {
                    sortedDataMedia2.push(dataMedia[i]);
                }
            }


            console.log(sortedDataMedia2);
            ...
            allMedia.forEach((media, index) => {
                media.addEventListener('click', (e) => {
                    ...
                    let data = dataMedia[selectedIndex];
                    console.log(data);
                });
            });*/
            ...
            function updateLightbox() {
                ...
                console.log(currentMedia.tagName);
